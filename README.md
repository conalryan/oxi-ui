...
- **@oxi-ui/core** - Core TypeScript library with authentication, feature flags, permissions, and
  utilities
- **@oxi-ui/theme** - Design tokens and CSS theme with light/dark mode support
- **@oxi-ui/web-components** - Lit-based web components with React and Angular wrappers (requires
  `@oxi-ui/theme`)
...
Design tokens and CSS theme system with automatic light/dark mode support using modern CSS
`light-dark()` syntax.

#### Installation

```bash
bun add @oxi-ui/theme
```

#### CSS Import (Recommended)

```typescript
// Import the CSS file for automatic theme support
import '@oxi-ui/theme/css';
```

This provides all CSS custom properties (`--oxi-*`) and automatically switches between light/dark
themes based on the user's system preference.

#### Programmatic Tokens

```typescript
import { lightColors, darkColors, theme } from '@oxi-ui/theme/tokens';
import { cssVars } from '@oxi-ui/theme';
...
> **Note:** `@oxi-ui/web-components` has a peer dependency on `@oxi-ui/theme`. Install both packages
> to ensure proper styling.

```bash
bun add @oxi-ui/web-components @oxi-ui/theme
```

#### Setup

```typescript
// Import theme CSS first for styling
import '@oxi-ui/theme/css';

// Then import web components
...
- **Versioning**: [Changesets](https://github.com/changesets/changesets) - Version management and
  publishing
- **Linting**: [Oxlint](https://oxc.rs/docs/guide/usage/linter.html) - Fast Rust-based linter
- **formatting**: [Oxfmt](https://oxc.rs/docs/guide/usage/formatter.html) - Fast Rust-based
  formatter
...
Open this repository in VS Code and use the "Reopen in Container" command for a fully configured
...
- **[Changesets](https://github.com/changesets/changesets)** - Versioning, changelogs, and
  publishing
