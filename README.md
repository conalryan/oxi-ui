...
Pilotingical Front-End library generated by AI - A modern monorepo using Bun + Vite + OXC.

## Overview

Piloting is a frontend monorepo that provides:

- **@piloting/core** - Core TypeScript library with authentication, feature flags, permissions, and utilities
- **@piloting/web-components** - Lit-based web components with React and Angular wrappers
...
import { isAuthorized } from '@piloting/core/authentication';
import { isEnabled } from '@piloting/core/feature-flags';
import { hasPermission } from '@piloting/core/permissions';
import { debounce, throttle } from '@piloting/core/utils';

// Or import everything
import { isAuthorized, isEnabled, hasPermission, debounce } from '@piloting/core';
```
...
```typescript
import { initializeFeatureFlags, isEnabled } from '@piloting/core/feature-flags';
...
```html
<script type="module">
  import '@piloting/web-components';
</script>
...
```tsx
import { PilotingButton, PilotingTextField } from '@piloting/web-components/react';
...
```html
<piloting-button-wrapper variant="primary" (pilotingClick)="handleClick($event)">
  Submit
...
│   │   │   └── utils/
│   │   └── vite.config.ts
│   └── web-components/  # @piloting/web-components
│       ├── src/
│       │   ├── button/
│       │   ├── text-field/
│       │   ├── react/
...
docker build --target development -t piloting-dev .

# Run tests
docker build --target test -t piloting-test .

# Run linting
docker build --target lint -t piloting-lint .

# Full CI pipeline
docker build --target ci -t piloting-ci .

# Production (built packages only)
docker build --target production -t piloting-prod .
```
