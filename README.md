...
- **@oxi-ui/web-components** - Lit-based web components with React and Angular wrappers (requires `@oxi-ui/theme`)
...
Design tokens and CSS theme system with automatic light/dark mode support using modern CSS `light-dark()` syntax.
...
This provides all CSS custom properties (`--oxi-*`) and automatically switches between light/dark themes based on the user's system preference.
...
> **Note:** `@oxi-ui/web-components` has a peer dependency on `@oxi-ui/theme`. Install both packages to ensure proper styling.
